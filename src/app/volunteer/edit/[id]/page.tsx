...
export default function EditVisitPage({
  ...
  const [foodItems, setFoodItems] = useState<{ item: string }[]>([]);
  ...
  useEffect(() => {
    supabase
      .from("visits")
      .select("visit_date, visit_time, is_recurring, bringing_meal, bringing_groceries, notes, volunteer_id, volunteers(name, email), food_items(item_name)")
      .eq("id", id)
      .single()
      .then(({ data: raw }) => {
        const data = raw as unknown as {
          ...
          notes: string | null;
          volunteer_id: string;
          volunteers: { name: string; email: string | null } | null;
          food_items: { item_name: string }[];
        } | null;
        if (data) {
          const guestMatch = data.notes?.match(/^Guest:\s*(.+)$/);
          setVolunteerId(data.volunteer_id);
          setForm({
            name: data.volunteers?.name ?? "",
            email: data.volunteers?.email ?? "",
            guestName: guestMatch ? guestMatch[1] : "",
            date: data.visit_date,
            time: data.visit_time.slice(0, 5),
            isRecurring: data.is_recurring,
          });
          setFoodItems(data.food_items.map((fi) => ({ item: fi.item_name })));
          // Set calendar month to visit month
          const [y, m] = data.visit_date.split("-").map(Number);
  ...
  function addFoodItem() {
    ...
    setFoodItems((prev) => [...prev, { item: newItem.trim() }]);
    setNewItem("");
  }
